...
if (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    $scriptPath = Join-Path -Path $ENV:TEMP -ChildPath "winutil.ps1"
    $scriptContent = Get-Content -Path $MyInvocation.MyCommand.Path
    $scriptContent | Set-Content -Path $scriptPath

    if ($wtInstalled) {
        Start-Process wt.exe -ArgumentList "$powershellcmd -ExecutionPolicy Bypass -NoProfile -File $scriptPath -Run" -Verb RunAs
    } else {
        Start-Process $powershellcmd -ArgumentList "-ExecutionPolicy Bypass -NoProfile -File $scriptPath -Run" -Verb RunAs
    }
